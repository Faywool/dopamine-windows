...
namespace CSCore.Codecs
{
    ...
    public class CodecFactory
    {
        ...
        ///     获取<see cref =" CodecFactory" />类的默认单例实例。
        /// </summary>
        ...
        ///     获取英语文件过滤器。 例如，可以使用该过滤器。 与OpenFileDialog结合使用。
        /// </summary>
        ...
        ///     内部使用的密钥将<paramref name ="codec" />保存在 <see cref ="Dictionary{TKey,TValue}" />。 这通常是关联的文件扩展名。 例如：mp3编解码器,使用字符串“ mp3”作为其键。
        /// </param>
        /// <param name="codec"><see cref="CodecFactoryEntry" /> which provides information about the codec.提供有关编解码器的信息。</param>
        ...
        ///     返回一个完全初始化的<see cref="IWaveSource" />实例，该实例能够解码指定的文件。 如果指定的文件无法解码，此方法将引发<see cref=" NotSupportedException" />。
        /// </summary>
        /// <param name="filename">Filename of the specified file.指定文件的文件名。</param>
        /// <returns>Fully initialized <see cref="IWaveSource" /> instance which is able to decode the specified file.实例，它可以解码指定的文件。</returns>
        /// <exception cref="NotSupportedException">The codec of the specified file is not supported.不支持指定文件的编解码器。</exception>
        public IWaveSource GetCodec(string filename)
        {
            ...
            string extension = Path.GetExtension(filename).Remove(0, 1); //get the extension without the "dot".得到没有"."的扩展名。
        ...
        ///     返回完全初始化的<see cref="IWaveSource" />实例，该实例能够对指定的<paramref name="uri" />后面的音频源进行解码。 如果指定的音频源无法解码，则此方法将引发<see cref="NotSupportedException" />。
        /// </summary>
        /// <param name="uri">Uri which points to an audio source.指向音频源的Uri。</param>
        /// <returns>Fully initialized <see cref="IWaveSource" /> instance which is able to decode the specified audio source.实例，它可以解码指定的音频源。</returns>
        /// <exception cref="NotSupportedException">The codec of the specified audio source is not supported.不支持指定音频源的编解码器。</exception>
...
#pragma warning restore 618
                ...
                throw;
                //better throw the exception of the MediaFoundationDecoder. We just try to use the Mp3WebStream class since a few mp3 streams are not supported by the mediafoundation.
                //最好抛出MediaFoundationDecoder的异常。 我们只是尝试使用Mp3WebStream类，因为mediafoundation不支持一些mp3流。
            }
        }
        ...
        ///     返回所有支持的编解码器的所有通用文件扩展名。 请注意，其中某些文件扩展名属于多个编解码器。
        ///     这意味着某些文件扩展名为abc的文件可能会被解码，而另一些文件扩展名为abc的文件可能无法解码。
        /// </summary>
        /// <returns>Supported file extensions.支持的文件扩展名。</returns>
